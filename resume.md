...
| P6 | In progress | `wm-backend-wayland` provides an optional Smithay 0.7 nested adapter boundary and compiles both disabled and enabled. A shared configure ledger provides serials, supersession, acknowledgement, expiry, and unmap cleanup; headless exercises it deterministically. The renderer frame ledger distinguishes render from presentation completion and retains ownership until presentation completes. `NestedWaylandServer` publishes compositor, shared-memory, seat, and xdg-shell globals; it accepts/dispatches clients, maps Smithay configure serials into `XdgLifecycle`, emits normalized lifecycle events, renders surface trees with frame callbacks, and forwards nested host keyboard and pointer input. Pointer hit testing is temporarily limited to the first toplevel at its initial origin. Its default tests pass; the enabled feature checks and strict clippy successfully but cannot link tests on this host until `libxkbcommon` development files are installed. Add cursor display and output metadata, geometry-aware input routing through the composition root, and adapter-backed frame/buffer completion while retaining the independent headless path. |
