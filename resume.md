...
Last completed step: `feat(p4): preserve provider state`.

## Resume next

Continue **P4 — Provider-defined layouts**. `wm-core` stores only opaque `LayoutId` values and calls the neutral `LayoutEngine` contract. `wm-layout` discovers arbitrary Lua provider IDs from configuration, validates declarations before registry activation, enforces bounded versioned callbacks, retains individual last-good providers on reload failure, and falls back to generic recovery placement without recognizing any shipped layout name. A successful calculation returns validated geometry, an optional complete back-to-front paint order, and JSON-compatible provider state. `wm-runtime` synchronizes accepted configuration generations to that registry at a caller-selected safe boundary and exposes generic selection by opaque ID; the headless runtime polls it every 150 ms. The complete API is in `docs/layout-provider-api.md`. The bundled Lua files are examples only. Next, define provider-defined interaction callbacks.
...
| P4 | In progress | The core stores opaque layout IDs and speaks only the neutral layout contract. Configuration discovers any named Lua provider, while the provider runtime validates declarations and callbacks, bounds execution, retains individual last-good providers, and uses generic recovery placement if needed. A provider can return a complete validated paint order and JSON-compatible state. `wm-runtime` synchronizes accepted configuration generations at a safe boundary and lets callers select any opaque provider ID; the headless runtime polls configuration every 150 ms. The shipped Lua files are defaults/examples, not WM-defined layout types. Provider-defined interaction callbacks remain. |
