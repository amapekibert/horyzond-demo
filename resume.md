...
| P6 | In progress | `wm-backend-wayland` provides an optional Smithay 0.7 nested adapter boundary and compiles both disabled and enabled. A shared configure ledger provides serials, supersession, acknowledgement, expiry, and unmap cleanup; headless exercises it deterministically. The renderer frame ledger distinguishes render from presentation completion and retains ownership until presentation completes. Implement minimal xdg-shell lifecycle, event-loop timeout enforcement, GLES rendering, adapter-backed frame/buffer releases, and composition-root selection; retain the independent headless path. |
