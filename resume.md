...
| P5 | In progress | Modal bindings, configuration-loaded deterministic rules evaluated after committed metadata changes, bounded configuration-backed `on_startup`, `on_reload`, and `on_window_open` hooks, IPC framing, bounded subscriptions, owner-only Unix IPC transport, private per-configuration sockets, status/config status routing, window/workspace queries, configured layout selection, camera transactions, explicit reload, and bounded pending spawn controls exist. Offline `horyctl config check` and online `horyctl status|windows|workspaces|layout select ID|camera set X Y ZOOM|config reload|spawn <pending EXECUTABLE [ARG...]|cancel>` exist. Add opaque-action dispatch and the workspace-change hook trigger. |
