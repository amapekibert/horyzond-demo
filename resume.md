...
| P5 | In progress | Modal bindings, deterministic rules, bounded hook policy, IPC framing, bounded subscriptions, owner-only Unix IPC transport, private per-configuration sockets, status/config status routing, window/workspace queries, offline `horyctl config check`, and online `horyctl status|windows|workspaces` exist. Add profile selection, camera commands, configuration reload, pending spawn, and configuration-backed rules/hooks. |
