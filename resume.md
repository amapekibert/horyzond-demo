...
| P6 | In progress | `wm-backend-wayland` provides an optional Smithay 0.7 nested adapter boundary and compiles both disabled and enabled. Neutral configure transactions are serial-bearing, explicitly acknowledged, supersede stale requests, and are tested headlessly. Implement minimal xdg-shell lifecycle, GLES rendering, frame/buffer completion ordering, timeout policy, and composition-root selection; retain the independent headless path. |
