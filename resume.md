...
| P5 | In progress | Modal bindings, configuration-loaded deterministic rules evaluated after committed metadata changes, bounded configuration-backed `on_startup`, `on_reload`, `on_window_open`, and `on_workspace_change` hooks, IPC framing, bounded subscriptions, owner-only Unix IPC transport, private per-configuration sockets, status/config status routing, window/workspace queries and switching, configured layout selection, camera transactions, explicit reload, and bounded pending spawn controls exist. Offline `horyctl config check` and online `horyctl status|windows|workspaces|workspace switch ID|layout select ID|camera set X Y ZOOM|config reload|spawn <pending EXECUTABLE [ARG...]|cancel>` exist. Add deliberate opaque-action dispatch. |
