...
| P6 | In progress | `wm-backend-wayland` provides an optional Smithay 0.7 nested adapter boundary and compiles both disabled and enabled. A shared configure ledger provides serials, supersession, acknowledgement, expiry, and unmap cleanup; headless exercises it deterministically. The renderer frame ledger distinguishes render from presentation completion and retains ownership until presentation completes. A feature-independent `XdgLifecycle` enforces registration, configure/acknowledge/initial-commit ordering, destruction cleanup, and timeout recovery. Its default tests pass; the enabled feature checks successfully but cannot link tests on this host until `libxkbcommon` development files are installed. Connect it to minimal real xdg-shell callbacks and an event-loop clock, implement GLES rendering with adapter-backed frame/buffer releases, and select it from the composition root; retain the independent headless path. |
