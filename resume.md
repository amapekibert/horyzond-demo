...
| P5 | In progress | Modal bindings, configuration-loaded deterministic rules, bounded hook policy, IPC framing, bounded subscriptions, owner-only Unix IPC transport, private per-configuration sockets, status/config status routing, window/workspace queries, configured layout selection, camera transactions, explicit reload, offline `horyctl config check`, and online `horyctl status|windows|workspaces|layout select ID|camera set X Y ZOOM|config reload` exist. Integrate rule results on lifecycle metadata events, then add pending spawn and configuration-backed hooks. |
