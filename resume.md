...
| P5 | In progress | Modal bindings, configuration-loaded deterministic rules evaluated after committed metadata changes, bounded hook policy, IPC framing, bounded subscriptions, owner-only Unix IPC transport, private per-configuration sockets, status/config status routing, window/workspace queries, configured layout selection, camera transactions, explicit reload, and bounded pending spawn controls exist. Offline `horyctl config check` and online `horyctl status|windows|workspaces|layout select ID|camera set X Y ZOOM|config reload|spawn <pending EXECUTABLE [ARG...]|cancel>` exist. Add configuration-backed hooks and a deliberate opaque-action dispatcher. |
